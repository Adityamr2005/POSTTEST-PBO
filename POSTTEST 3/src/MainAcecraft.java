...
public class MainAcecraft {
    ...
    public static void main(String[] args) {
        int pilihan;
        do {
            System.out.println("\n=== SISTEM PENYIMPANAN DATA ACECRAFT (INHERITANCE) ===");
            System.out.println("1. Tambah Data (Player/Admin)");
    ...
    static void tampilkanData() {
        if(database.isEmpty()) {
            ...
            for(Acecraft acc : database) {
                System.out.println("-----------------------");
                acc.tampilkanData(); // Polymorphism: memanggil override method di subclass
            }
        }
    }
    ...
    static void updateData() {
        ...
        for(Acecraft akun : database) {
            if(akun.getUid().equals(uid)) {
                ...
                System.out.print("Server baru   : "); akun.setServer(input.nextLine());
